;;; company-tailwindcss.el --- A company back-end for Tailwindcss

;; Copyright (C) 2024 by Vernon Grant.

;; Author: Vernon Grant <info@vernon-grant.com>
;; Version: 0.0.0
;; Package-Requires: ((emacs "27.1"))
;; Keywords: company, package
;; Homepage: https://github.com/VernonGrant/company-tailwindcss
...
;; Credit to https://github.com/Azzam-dev, for creating the intial class list.
...
(defun company-tailwindcss (command &optional arg &rest ignored)
  ...
  (cl-case command
    ...
    (prefix (and
             ;; Enable tailwind completions in all of these major modes.
             ;; TODO: Make this a setting.
             (seq-contains-p '(web-mode php-mode css-mode css-ts-mode html-mode) major-mode)
             (company-grab-symbol)))
    (candidates
     (seq-filter (lambda (c) (string-prefix-p arg c)) company-tailwindcss-classes))))

(add-to-list 'company-backends 'company-tailwindcss)

;; TODO: Refactor this so it can be handled better.
;; (setq eglot-stay-out-of '(company))
;; Egot integration.
(defun company-tailwindcss-setup ()
  (add-to-list 'company-backends '(company-tailwindcss company-capf)))
(add-hook 'eglot-managed-mode-hook 'company-tailwindcss-setup)
